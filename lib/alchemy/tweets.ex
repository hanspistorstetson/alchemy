defmodule Alchemy.Tweets do
  ...
  def get_tweets_by_user(userid) do
    user =
      Alchemy.Accounts.get_user!(userid)
    |> Repo.preload([:tweets])
    user.tweets
  end

  def get_tweets_contains(string) do
    tweets = Repo.all(from t in Tweet, where: ilike(t.text, ^"%#{string}%"))
    tweets
  end
end
